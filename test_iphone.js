...
async function testIPhoneViewport() {
  ...
  try {
    ...
    const aiCard = await page.$('.mode-card:has(.mode-title h3:contains("AI Challenge"))');
    if (aiCard) {
      await aiCard.click();
    ...
    console.log('Testing Multiplayer Game Creation...');
    const multiplayerCard = await page.$('.mode-card:has(.mode-title h3:contains("Multiplayer"))');
    if (multiplayerCard) {
      await multiplayerCard.click();
      await page.waitForTimeout(1000);
      
      // Take screenshot of multiplayer game setup
      await page.screenshot({
        path: path.join(__dirname, 'screenshots', 'multiplayer-game-setup-iphone.png'),
        fullPage: true
      });
      console.log('✓ Multiplayer game setup screenshot saved');
    }
