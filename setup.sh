...
auto_populate_firebase_config() {
    ...
    if ! command -v jq &> /dev/null; then
        log "WARNING" "jq not available for JSON parsing - trying alternative method"
        return fetch_firebase_config_without_jq
    fi
    
    # Try to get project info
    local project_info=$(firebase projects:list --json 2>/dev/null)
    ...
    # Check if project exists
    if ! echo "$project_info" | jq -e ".projects[] | select(.projectId == \"$PROJECT_ID\")" &> /dev/null; then
        log "WARNING" "Project $PROJECT_ID not found in Firebase projects list"
        return 1
    fi
    
    # Try to get web app list first
    local apps_list=$(firebase apps:list --project "$PROJECT_ID" 2>/dev/null)
    if [[ -z "$apps_list" ]] || echo "$apps_list" | grep -q "No apps found"; then
    ...
    local app_id=$(firebase apps:list --project "$PROJECT_ID" 2>/dev/null | grep "WEB" | awk '{print $2}' | head -1)
    if [[ -z "$app_id" ]]; then
        log "WARNING" "Could not find web app ID"
        return 1
    fi
    
    log "INFO" "Found web app: $app_id"
    
    # Get SDK configuration using the app ID
    local sdk_config=$(firebase apps:sdkconfig WEB "$app_id" --project "$PROJECT_ID" 2>/dev/null)
    if [[ -z "$sdk_config" ]]; then
        log "WARNING" "Could not fetch SDK configuration"
        return 1
    fi
    
    # Extract values using jq
    local api_key=$(echo "$sdk_config" | jq -r '.apiKey // empty')
    local auth_domain=$(echo "$sdk_config" | jq -r '.authDomain // empty')
    local storage_bucket=$(echo "$sdk_config" | jq -r '.storageBucket // empty')
    local messaging_sender_id=$(echo "$sdk_config" | jq -r '.messagingSenderId // empty')
    local project_id=$(echo "$sdk_config" | jq -r '.projectId // empty')
    local actual_app_id=$(echo "$sdk_config" | jq -r '.appId // empty')
    
    # Validate required fields
    if [[ -z "$api_key" ]] || [[ "$api_key" == "null" ]] || [[ "$api_key" == "empty" ]]; then
        log "WARNING" "Could not extract API key from configuration"
        return 1
    fi
    
    if [[ -z "$actual_app_id" ]] || [[ "$actual_app_id" == "null" ]] || [[ "$actual_app_id" == "empty" ]]; then
        log "WARNING" "Could not extract App ID from configuration"
        return 1
    fi
    ...
    log "INFO" "Successfully extracted Firebase configuration:"
    log "INFO" "- API Key: ${api_key:0:20}..."
    log "INFO" "- Auth Domain: $auth_domain"
    log "INFO" "- Storage Bucket: $storage_bucket"
    log "INFO" "- App ID: $actual_app_id"
...
# Firebase Configuration (Auto-populated)
VITE_FIREBASE_API_KEY=$api_key
VITE_FIREBASE_AUTH_DOMAIN=$auth_domain
VITE_FIREBASE_PROJECT_ID=$PROJECT_ID
VITE_FIREBASE_STORAGE_BUCKET=$storage_bucket
VITE_FIREBASE_MESSAGING_SENDER_ID=$messaging_sender_id
VITE_FIREBASE_APP_ID=$actual_app_id
...
# Function to fetch Firebase config without jq (fallback method)
fetch_firebase_config_without_jq() {
    log "INFO" "Using fallback method to fetch Firebase configuration..."
    
    # Get the app ID from the list (basic parsing)
    local app_id=$(firebase apps:list --project "$PROJECT_ID" 2>/dev/null | grep "WEB" | awk '{print $2}' | head -1)
    if [[ -z "$app_id" ]]; then
        log "INFO" "No web app found, creating one..."
        if create_firebase_web_app; then
            log "SUCCESS" "Web app created successfully"
            sleep 2
            app_id=$(firebase apps:list --project "$PROJECT_ID" 2>/dev/null | grep "WEB" | awk '{print $2}' | head -1)
        else
            log "WARNING" "Failed to create web app"
            return 1
        fi
    ...
    # Extract values using basic text parsing (fallback for systems without jq)
    local api_key=$(echo "$sdk_config" | grep '"apiKey"' | cut -d'"' -f4)
    local auth_domain=$(echo "$sdk_config" | grep '"authDomain"' | cut -d'"' -f4)
    local storage_bucket=$(echo "$sdk_config" | grep '"storageBucket"' | cut -d'"' -f4)
    local messaging_sender_id=$(echo "$sdk_config" | grep '"messagingSenderId"' | cut -d'"' -f4)
    local actual_app_id=$(echo "$sdk_config" | grep '"appId"' | cut -d'"' -f4)
    ...
    log "INFO" "Successfully extracted Firebase configuration (fallback method)"
...
EOF
    ...
    mv "$temp_file" "$env_file"
    log "SUCCESS" "Firebase configuration updated with actual values (fallback method)"
    return 0
}
...
create_firebase_web_app() {
    ...
    local app_name="${PROJECT_NAME:-Chaupar}"
    
    # Use interactive creation method that works more reliably
    if firebase apps:create web --project "$PROJECT_ID" >/dev/null 2>&1 <<EOF
$app_name

EOF
    then
        log "SUCCESS" "Web app '$app_name' created successfully"
        return 0
    else
        log "WARNING" "Failed to create web app automatically"
        log "INFO" "You can create it manually at: https://console.firebase.google.com/project/$PROJECT_ID/settings/general"
        return 1
    fi
}
